...
pre_test <- function(train_pheno,train_geno,pre_test_num){
  ...
  for(hoop in 1:pre_test_num){
    ...
    CCR_LM <- function(n,y.train, X.train, X.test, chunk=NULL, cc_num=10){
      lm1 = fastLmPure(X=matrix(cbind(1, XX1[, 1:n]), ncol=n+1), y=y.train, method=2)
      Predition_Pheno <- XX1.1[, 1:n]%*%(lm1$coefficients[-1])
      return(Predition_Pheno)
    }
    CCR_SVM <- function(n,y.train, X.train, X.test, chunk=NULL, cc_num=10){
      mm<-ksvm(x= XX1[,1:n], y=y.train, kernel="rbfdot")
      Predition_Pheno <- predict(mm, XX1.1[,1:n])
      return(Predition_Pheno)
    }
    ...
    Best_LM <- fastLmPure(X=cbind(1, XX1[, index]), y=y_train_data, method=2)
    Min_LM <- fastLmPure(X=cbind(1, XX1[, index1]), y=y_train_data, method=2)
    Best_SVM <- ksvm(x= XX1[, index], y=y_train_data, kernel="rbfdot")
    Min_SVM <- ksvm(x= XX1[, index1], y=y_train_data, kernel="rbfdot")
    ...
    CCR_Best_SVM <- predict(Best_SVM, XX1.1[, index])
    CCR_Min_SVM <- predict(Min_SVM, XX1.1[, index1])
